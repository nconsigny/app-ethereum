...
bool compute_schema_hash(void) {
    ...
    while (struct_ptr != NULL) {
        hash_byte('"', (cx_hash_t *) &hash_ctx);
        hash_nbytes((uint8_t *) struct_ptr->name, strlen(struct_ptr->name), (cx_hash_t *) &hash_ctx);
        ...
        while (field_ptr != NULL) {
            ...
            hash_nbytes((uint8_t *) field_ptr->key_name, strlen(field_ptr->key_name), (cx_hash_t *) &hash_ctx);
