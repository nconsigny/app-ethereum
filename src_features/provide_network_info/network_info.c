...
bool verify_network_info_struct(const s_network_info_ctx *context) {
    ...
    if (DYNAMIC_NETWORK_INFO[g_current_network_slot]) {
        app_mem_free(DYNAMIC_NETWORK_INFO[g_current_network_slot]);
    }
    DYNAMIC_NETWORK_INFO[g_current_network_slot] = app_mem_alloc(sizeof(network_info_t));
    ...
    g_network_icon_hash = app_mem_alloc(CX_SHA256_SIZE);
    if (g_network_icon_hash == NULL) {
        PRINTF("Memory allocation failed for icon hash\n");
        return false;
    }
