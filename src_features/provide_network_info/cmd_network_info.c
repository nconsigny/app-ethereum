...
static bool parse_icon_buffer(void) {
    ...
    app_mem_free(g_network_icon_hash);
...
static uint16_t handle_first_icon_chunk(const uint8_t *data, uint8_t length) {
    ...
    g_icon_bitmap[g_current_network_slot] = app_mem_alloc(img_len);
    ...
    explicit_bzero(g_icon_bitmap[g_current_network_slot], img_len);
...
void network_info_cleanup(uint8_t slot) {
    if (slot == MAX_DYNAMIC_NETWORKS) {
        ...
        for (slot = 0; slot < MAX_DYNAMIC_NETWORKS; slot++) {
            if (g_icon_bitmap[slot]) {
                app_mem_free(g_icon_bitmap[slot]);
                g_icon_bitmap[slot] = NULL;
            }
            if (DYNAMIC_NETWORK_INFO[slot]) {
                app_mem_free(DYNAMIC_NETWORK_INFO[slot]);
                DYNAMIC_NETWORK_INFO[slot] = NULL;
            }
        }
    } else {
